# Paper 1 ACPC/IR/DR rebuild scripts
...
LeWM pair rows ───────────────> cross_task_selective_rule ──> IR/DR summary
PLDM frozen rows ─────────────> plot_pldm_sweep_diagnostics
IR/DR summary + stressor rows ─> build_cross_stressor_ir_dr_comparison
planner summary + sweep rows ─> build_acpc_submission_assets
Figure 1 input PNGs ──────────> plot_acpc_ir_dr_overview
```
...
core IR/DR path without changing the checkout, use temporary outputs:
...
Some frozen raw files predate the reader-facing names and retain legacy
columns. `ir_dr_compat.py` maps them without changing stored values:
...
New scripts and generated displays use IR/DR terminology. The compatibility
layer exists only so immutable raw evidence remains readable and auditable.
