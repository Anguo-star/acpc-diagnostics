...
class JEPA(nn.Module):
    ...
    @staticmethod
    def _broadcast_goal_to_prediction(
        goal_emb: torch.Tensor, pred_emb: torch.Tensor
    ) -> torch.Tensor:
        """Broadcast encoded goals across the planner's sample dimension."""
        if goal_emb.ndim == pred_emb.ndim - 1:
            goal_emb = goal_emb.unsqueeze(1)
        if goal_emb.ndim != pred_emb.ndim:
            raise ValueError(
                f"goal_emb ndim {goal_emb.ndim} is incompatible with "
                f"pred_emb ndim {pred_emb.ndim}"
            )
    ...
    def _inference_history_size(
        ...
        """Resolve the history length stored by checkpoints from either API."""
