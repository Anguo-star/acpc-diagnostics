...
- **Seed-3072 strongest-only unseen-perturbation pilot**: `assets/paper1_data/unseen_origin_vs_std008_strongest_tworoom.json` and `assets/paper1_data/unseen_origin_vs_std008_strongest_reacher.json` (review artifacts covering 4 tasks x 2 std keys x 2 stress families with no-op plus blur k=15 / resize factor 0.25)
- **Seed-3073/3074 strongest-only unseen-perturbation lockbox artifacts**: `assets/paper1_data/unseen_origin_vs_std008_strongest_s3073.json` and `assets/paper1_data/unseen_origin_vs_std008_strongest_s3074.json` (independent training-seed review artifacts; each covers 4 tasks x 2 std keys x 2 stress families, `missing=0`)
...
|---|---|---|
| `canonical_evals_20260517.json` | LeWM evaluation aggregate | `53b8af39c1c3851f6351fefd5a50ed62c49193311b287e09478bec5d63ac712f` |
...
| `acpc_basin_diagnostics.json` | LeWM Gaussian ACPC basin | `2b5db3508d51e2dbc93e4cb774662d5577dde36a755a2ddc273bfade438e90bc` |
| `canonical_evals_pldm_20260522.json` | PLDM evaluation aggregate | `0c642f956b687ae7982f12b84482673bcc271f5eaf1168f5c78e835095c3057e` |
...
| `canonical_full_diagnostics_pldm_20260523.json` | PLDM five-layer diagnostics | `f51416e3d7684f823de987769e3cd40f3d64c803a246dfb18170de6c3aba6984` |
| `acpc_basin_diagnostics_pldm.json` | PLDM Gaussian ACPC basin | `cd0c19ad12293595e7c5898d365a329f1ed54e84c0b7562a409b5b52e133dded` |
...
| `canonical_blur_baselines_20260523.json` | Eval-only blur sanity check | `c092c3f5f751fd25c011005c2166bb3a037943a3b8a01e4d6306f0e7dba02b78` |
...
- Canonical lookup key for portability: use `subdir`
  - JSON `path`, `run_path`, and `model_file` fields are stored relative to `DATA_ROOT` when they refer to released checkpoints
...
- `unseen_origin_vs_std008_strongest_s3073.json` and `unseen_origin_vs_std008_strongest_s3074.json` store the independent-training-seed strongest-only blur/resize eval summaries for `std_key` 0.0 and 0.08. They are built from the manifests written by `tools/paper1_unseen_eval_grid.py`.
- `unseen_phase0_acpc_subset.json` stores the representative unseen follow-up subset: TwoRoom/Reacher blur as positive-transfer cases and PushT/Cube resize as boundary cases, each comparing `std_key` 0.0 vs 0.08 for training seeds 3073/3074 with clean-goal Phase-0 paired diagnostics. It joins diagnostics with strongest-only unseen eval scores and is a review artifact for reproduction.
- Released JSON checkpoint paths are portable relative paths under `DATA_ROOT`; reruns should resolve checkpoints from `DATA_ROOT` plus the relative task roots above.
